...
def eval_policy(env_params, checkpoint, n_eval_episodes, max_steps, seed, render):
    ...
    policy = DQNAgent(state_size, action_size, num_agents=5)
    policy.load(project_root / 'checkpoints', 0, 1.0)
    ...
    tree_observation = TreeObservation(max_depth=observation_tree_depth)
    ...
    for episode_idx in range(n_eval_episodes):
        ...
        for step in range(max_steps - 1):
            ...
            for agent in env.get_agent_handles():
                if obs[agent]:
                    preproc_timer.start()
                    agent_obs[agent] = normalize_observation(obs[agent], observation_tree_depth, zero_center=True)
                    preproc_timer.end()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--file", help="checkpoint to load", required=False, type=str)
    ...
    parser.add_argument("--render", help="render a single episode", default=True, type=bool)
